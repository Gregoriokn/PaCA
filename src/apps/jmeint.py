...
def cleanup_variant_files(variant_hash, config, preserve_logs=True):
    ...
    app_name = config.get("app_name", "jmeint")
...
def run_profiling_stage(resume_context, base_config, status_monitor):
    ...
    try:
        ...
        try:
            ...
            original_filepath = config.get("tritri_source_file")
            if variant_file and original_filepath and os.path.exists(variant_file) and os.path.exists(original_filepath):
                ...
                if operations_per_line:
                    ...
                    operacoes_path = os.path.join(operacoes_dir, f"operacoes_{current_tritri_hash}.json")
...
def simulate_variant(current_tritri_filepath, current_tritri_hash, base_config, status_monitor, only_spike=False):
    ...
    spike_log_file = os.path.join(logs_dir, f"{app_name}_{current_tritri_hash}.json")
