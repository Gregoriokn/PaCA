...
def simulate_variant(variant_file, variant_hash, base_config, status_monitor):
    ...
    app_name = config.get("app_name", "nova_app")
    ...
    spike_log_file = os.path.join(config["logs_dir"], f"{app_name}_{variant_hash}.json")
    ...
    with TempFiles([spike_log_file, dump_file]):
        ...
        # Passo 4: Executar o Prof5Fake
        # (versão simplificada que usa o log do Spike diretamente)
        if not os.path.exists(prof5_report_path):
            os.makedirs(os.path.dirname(prof5_report_path), exist_ok=True)
        
        instrucoes_dict = contar_instrucoes_log(spike_log_file)
        resultados_energia = avaliar_modelo_energia(instrucoes_dict, config.get("prof5_model"))
        
        with open(prof5_report_path, 'w') as f:
            json.dump(resultados_energia, f, indent=2)
        os.chmod(prof5_report_path, 0o666)
        
        prof5_time = sim_time  # Usar tempo de simulação comoaproximação
