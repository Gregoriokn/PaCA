        ...
        try:
            with open(debug_file, 'w') as f_debug:
                ...
                for variant_file, variant_hash in variants:
                    ...
                    logical_modified = []
                    for idx in modified_physical:
                        if idx in physical_to_logical:
                            logical_modified.append(physical_to_logical[idx])
                        elif (idx - 1) in physical_to_logical:
                            # Line after annotation - use the annotation line's logical mapping
                            logical_modified.append(physical_to_logical[idx - 1])
                        else:
                            logical_modified.append(idx)
                    logical_modified = sorted(set(logical_modified))
