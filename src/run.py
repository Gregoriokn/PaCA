...
def save_modified_lines_for_bruteforce(variant_file, original_file, variant_hash, app_module, config):
    ...
    try:
        ...
        # Garante que as listas tenham o mesmo tamanho para comparação segura
...
def main():
    ...
    # Tenta obter a configuração específica da aplicação e mesclá-la
    # para que esteja disponível em todo o fluxo de execução.
    # Isso é importante para funções como cleanup_variant_files quando chamadas a partir daqui.
    app_config_name = f"{args.app.upper()}_CONFIG"
    if hasattr(app_module, app_config_name):
        app_config = getattr(app_module, app_config_name)
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if result:
                            ...
                            # Salva as linhas modificadas para a variante bem-sucedida
                            original_source_file = app_module.get_pruning_config(execution_config)["source_file"]
